...
export class DisclaimerComponent implements OnInit {
  ...
  public disclaimForm: FormGroup = new FormGroup({
    header:this.header,
    pText1: this.pText1
  });
  ...
  getData(){
    this.http.get(''+this.data.apiDomainPathDash+'getDisclaimer') // Get privacy details
    .subscribe(data => {
      console.log(data);
      this.data.hideLoader();
      
      this.disclaimDetails = data;
      this.disclaimDetails = this.disclaimDetails.response[0];

      this.pText1.reset(this.disclaimDetails.content);
      this.header.reset(this.disclaimDetails.title);
    },
    err => {
      // Swal.fire(err);
    });
  ...
  disclaimSection(){
    ...
    this.http.post(''+this.data.apiDomainPathDash+'editDisclaimer',uploadData) // Set privacy details
    .subscribe(data => {
      console.log(data);
      this.uplodInfo = data;
      this.data.hideLoader();
