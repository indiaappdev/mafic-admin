...
export class TermsConditionsComponent implements OnInit {
  ...
  getData(){
    this.http.get(''+this.data.apiDomainPathDash+'getTermsAndConditions') // Get privacy details
    .subscribe(data => {
      console.log(data);
      this.data.hideLoader();
      
      this.termsDetails = data;
      this.termsDetails = this.termsDetails.response[0];

      this.pText1.reset(this.termsDetails.content);
      this.header.reset(this.termsDetails.title);
    },
    err => {
      // Swal.fire(err);
    });
  ...
  termsSection(){
    ...
    this.http.post(''+this.data.apiDomainPathDash+'editTermsAndConditions',uploadData) // Set privacy details
    .subscribe(data => {
      console.log(data);
      this.uplodInfo = data;
      this.data.hideLoader();
